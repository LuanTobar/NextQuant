    ...
    async def open_short(
        ...
        try:
            order_resp = await client.place_order(OrderRequest(
                ...
                quantity=quantity,
                type="market", time_in_force="gtc",
                trade_side="open",  # Futures: open a new short
            ))
            broker_order_id = order_resp.broker_id
            ...
            if pool and conn_id:
                await save_order(
                    pool, user_id, conn_id,
                    symbol, "SELL_SHORT", quantity, "market",
                    broker_order_id,
                    {
                        "source": "agent",
                        "signal": signal_data.get("signal"),
            ...
            entry_price = signal_data.get("current_price", 0)
            tracker.record_open(user_id, symbol, quantity, entry_price, side="short")
            ...
            return ExecutionResult(action="OPEN_SHORT", symbol=symbol, status="FAILED")
    ...
    async def close_short(
        ...
        try:
            order_resp = await client.place_order(OrderRequest(
                ...
                trade_side="close",  # Futures: close the short
            ))
            broker_order_id = order_resp.broker_id
            ...
            if pool and conn_id:
                await save_order(
                    pool, user_id, conn_id,
                    symbol, "BUY_COVER", quantity, "market",
                    broker_order_id,
                    {
                        "source": "agent",
                        "signal": signal_data.get("signal"),
            ...
            if position:
                ...
                pnl = (position.avg_entry_price - signal_data.get("current_price", 0)) * quantity
                risk_mgr.record_trade_pnl(user_id, pnl)
            ...
            logger.error(
                "Failed to close short",
                user_id=user_id, symbol=symbol, error=str(e),
            )
            return ExecutionResult(action="CLOSE_SHORT", symbol=symbol, status="FAILED")
